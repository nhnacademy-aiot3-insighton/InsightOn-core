...
class ForecastWeatherSchedulerTest {
    ...
    void cacheRefreshScheduler_success() {
        ...
        // when
        forecastWeatherScheduler.cacheRefreshScheduler();

        // then
        verify(weatherCacheService).refreshVillageForecast(anyInt(), anyInt(), anyString(), anyString());
    }

    @Test
    @DisplayName("단기예보 스케줄러 예외 발생 시 로그 출력 후 격리")
    void cacheRefreshScheduler_handlesException() {
        ...
        doThrow(new RuntimeException("Forecast error"))
                .when(weatherCacheService).refreshVillageForecast(anyInt(), anyInt(), anyString(), anyString());
