...
class GroupMemberRepositoryTest {
    ...
    void findByGroup_GroupIdAndUserId_success() {
        ...
        // when
        Optional<GroupMember> found = groupMemberRepository.findByGroup_GroupIdAndUserId(group.getGroupId(), 200L);
    ...
    void existsByGroup_GroupIdAndUserId_success() {
        ...
        // when
        boolean exists = groupMemberRepository.existsByGroup_GroupIdAndUserId(group.getGroupId(), 300L);
    ...
    void findAllByGroup_GroupId() {
        List<GroupMemberListResponse> list = groupMemberRepository.findAllByGroup_GroupId(1L);
        assertThat(list).hasSize(1);
    }

    @Test
    @DisplayName("그룹 ID와 사용자 ID로 존재 여부 확인")
    void existsByGroup_GroupIdAndUserId() {
        assertThat(groupMemberRepository.existsByGroup_GroupIdAndUserId(1L, 1L)).isTrue();
    }

    @Test
    @DisplayName("그룹 ID와 사용자 ID로 조회")
    void findByGroup_GroupIdAndUserId() {
        Optional<GroupMember> found = groupMemberRepository.findByGroup_GroupIdAndUserId(1L, 1L);
        assertThat(found).isPresent();
        assertThat(found.get().getUserId()).isEqualTo(1L);
    }
    ...
    void findByGroupMemberIdAndGroup_GroupId() {
        // SQL에서 삽입된 데이터의 groupMemberId가 1이라고 가정
        Optional<GroupMember> found = groupMemberRepository.findByGroupMemberIdAndGroup_GroupId(1L, 1L);
        assertThat(found).isPresent();
    }
    ...
    void deleteAllByGroup_GroupId() {
        groupMemberRepository.deleteAllByGroup_GroupId(1L);

        // 삭제 후 확인
        assertThat(groupMemberRepository.findAllByGroup_GroupId(1L)).isEmpty();
    }
