...
class DeviceServiceTest {
    ...
    void autoProvision_신규디바이스_생성() {
        ...
        DeviceEntity saved = DeviceEntity.builder()
                .deviceId(100L)
                .deviceEui("EUI-001")
                .build();
        given(deviceRepository.save(any(DeviceEntity.class))).willReturn(saved);
    ...
    void autoProvision_기존EUI_캐시복구만() {
        DeviceEntity existing = DeviceEntity.builder().deviceId(1L).deviceEui("EUI-001").build();
    ...
    void 조회_다른그룹이면_예외() {
        ...
        DeviceEntity device = DeviceEntity.builder().deviceId(1L).groupId(group).build();
    ...
    void 전체삭제_null_EUI_필터링() {
        DeviceEntity withEui = DeviceEntity.builder().deviceId(1L).deviceEui("EUI-001").build();
        DeviceEntity withoutEui = DeviceEntity.builder().deviceId(2L).deviceEui(null).build();
