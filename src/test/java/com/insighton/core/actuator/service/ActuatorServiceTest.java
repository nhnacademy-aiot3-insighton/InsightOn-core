...
import com.insighton.core.actuators.service.impl.ActuatorsServiceImpl;
...
class ActuatorServiceTest {
    ...
    void 생성_다른그룹location_거부() {
        ...
        ActuatorsRequest request = new ActuatorsRequest(99L, "에어컨", ActuatorType.AIRCON, Map.of("power", "OFF"));

        assertThrows(LocationNotFoundException.class,
                () -> actuatorsService.createActuator(1L, 10L, request));

        verify(actuatorsRepository, never()).save(any());
    }
    ...
    void 조회_다른그룹_소속아니면_404() {
        ...
        ActuatorsEntity entity = ActuatorsEntity.builder().actuatorId(1L).locationId(location).build();

        given(actuatorsRepository.findById(1L)).willReturn(Optional.of(entity));
        given(groupMembersService.validateGroupMembers(10L, 1L)).willReturn(manager());
    ...
    void 삭제_성공() {
        ...
        verify(actuatorsRepository).delete(entity);
    }
    ...
    void 전체삭제_그룹스코프() {
        ...
        verify(actuatorsRepository).deleteAllByLocationId_LocationIdIn(java.util.List.of());
    }
}