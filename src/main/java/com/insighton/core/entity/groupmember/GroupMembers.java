package com.insighton.core.entity.groupmember;
...
//            name = "group_id",
    ...
    /**
     * @Builder 패턴을 적용한 생성자
     * - 엔티티 최초 가입/생성 시 필요한 필드(groups, usersId, groupRole)만 빌더로 넘겨받아 안전하게 객체를 생성합니다.
     * - DB에서 자동 생성되는 PK(groupMemberId)와 @PrePersist로 자동 설정되는 가입 일자(joinedAt)는 제외하여 실수를 방지합니다.
     */
    @Builder
    public GroupMembers(Groups groups, Long userId, GroupRole groupRole){
        this.groups = groups;
        this.userId = userId;
