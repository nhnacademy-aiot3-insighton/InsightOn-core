package com.insighton.core.domain.location.entity;
...
public class Location {
     ...
     * - 엔티티 최초 가입/생성 시 필요한 필드(group, locationName, autoControlMode)만 빌더로 넘겨받아 안전하게 객체를 생성합니다.
     * - DB에서 자동 생성되는 PK(locationId)와 @PrePersist로 자동 설정되는 가입 일자(createdAt)는 제외하여 실수를 방지합니다.
     */
    @Builder
    public Location(Group group, String locationName, AutoControlMode autoControlMode) {
        this.group = group;
